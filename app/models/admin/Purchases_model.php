...
class Purchases_model extends CI_Model
{
    ...
    {
        // $q = $this->db->join('expent_types e','p.exType_id = e.id')->get_where('purchases p', array('p.id' => $id,'p.status' => 'expend'), 1);
        $q = $this->db->query("SELECT
                                    *
                                FROM
                                    `sma_purchases` `p`
                                JOIN `sma_expent_type` `e` ON `p`.`exType_id` = `e`.`id`
                                WHERE
                                    `p`.`id` = '".$id."'
                                AND `p`.`status` = 'expend' LIMIT 1");
        if ($q->num_rows() > 0) {
            return $q->row();
        }
        return FALSE;
