...
//generate pdf and force to download
    ...
    public function getExpenses()
    {
        ...
        $this->datatables
            ->select($this->db->dbprefix('purchases') . ".id as id,{$this->db->dbprefix('purchases')}.date as date, reference_no,{$this->db->dbprefix('expent_type')}.expend_name as ex_name,{$this->db->dbprefix('companies')}.name as category, grand_total as amount, note, CONCAT({$this->db->dbprefix('users')}.first_name, ' ', {$this->db->dbprefix('users')}.last_name) as user, attachment", false)
            ->from('purchases')
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
                ...
                if ($this->input->post('form_action') == 'export_excel') {
                    ...
                    $this->excel->getActiveSheet()->SetCellValue('C1', lang('expense Type'));
                    ...
                    foreach ($_POST['val'] as $id) {
                        ...
                        $this->excel->getActiveSheet()->SetCellValue('B' . $row, $expense->reference_no);
                        $this->excel->getActiveSheet()->SetCellValue('C' . $row, $expense->expend_name);
                        $this->excel->getActiveSheet()->SetCellValue('D' . $row, $this->sma->formatMoney($expense->grand_total));
