...
class system_settings extends MY_Controller
{
    ...
    {
        $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
        $bc = array(array('link' => base_url(), 'page' => lang('home')), array('link' => admin_url('system_settings'), 'page' => lang('system_settings')), array('link' => '#', 'page' => lang('location')));
        $meta = array('page_title' => lang('location'), 'bc' => $bc);
        $this->page_construct('settings/location', $meta, $this->data);
    }

    function add_location()
    {

        $this->form_validation->set_rules('name', lang("location_name"), 'trim|required|is_unique[location.name]|alpha_numeric_spaces');
        $this->form_validation->set_rules('slug', lang("slug"), 'trim|required|is_unique[location.slug]|alpha_dash');
        ...
        if ($this->form_validation->run() == true) {
            ...
            $this->session->set_flashdata('error', validation_errors());
            admin_redirect("system_settings/location");
        }

        if ($this->form_validation->run() == true && $this->settings_model->AddLocation($data)) {
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->load->view($this->theme . 'settings/add_location', $this->data);
    ...
    function GetLocation()
    {
        ...
        $this->datatables
            ...
            ->from("location")
            ->add_column("Actions", "<div class=\"text-center\"><a href='" . admin_url('system_settings/edit_location/$1') . "' data-toggle='modal' data-target='#myModal' class='tip' title='" . lang("edit_location") . "'><i class=\"fa fa-edit\"></i></a> <a href='#' class='tip po' title='<b>" . lang("delete_location") . "</b>' data-content=\"<p>" . lang('r_u_sure') . "</p><a class='btn btn-danger po-delete' href='" . admin_url('system_settings/delete_location/$1') . "'>" . lang('i_m_sure') . "</a> <button class='btn po-close'>" . lang('no') . "</button>\"  rel='popover'><i class=\"fa fa-trash-o\"></i></a></div>", "id");
    ...
    function edit_location($id = NULL)
    {
        ...
        $this->form_validation->set_rules('name', lang("location_name"), 'trim|required|alpha_numeric_spaces');
        $location_details = $this->site->getLocationByID($id);
        if ($this->input->post('name') != $location_details->name) {
            $this->form_validation->set_rules('name', lang("location_name"), 'required|is_unique[brands.name]');
        }
        $this->form_validation->set_rules('slug', lang("slug"), 'required|alpha_dash');
        if ($this->input->post('slug') != $location_details->slug) {
            $this->form_validation->set_rules('slug', lang("slug"), 'required|alpha_dash|is_unique[brands.slug]');
        }
        ...
        if ($this->form_validation->run() == true && $this->settings_model->updatelocation($id, $data)) {
            ...
            $this->data['error'] = validation_errors() ? validation_errors() : $this->session->flashdata('error');
            $this->data['modal_js'] = $this->site->modal_js();
            $this->data['location'] = $location_details;
            $this->load->view($this->theme . 'settings/edit_location', $this->data);
    ...
    {
        if ($this->settings_model->deletelocation($id)) {
            $this->sma->send_json(array('error' => 0, 'msg' => lang("location_deleted")));
            $this->load->view($this->theme . 'settings/location', $this->data);
        }else{
            $this->sma->send_json(array('error' => 0, 'msg' => lang("location_has_product")));
            $this->load->view($this->theme . 'settings/location', $this->data);
        }
    }
