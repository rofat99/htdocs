...
function loadItems() {
    ...
    if (localStorage.getItem('positems')) {
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var pos_head2 = '</h4><h6><div class="col-md-6 pull-left" style="text-align:left;font-size:11px !important;">T_Num: '+$('#select2-chosen-1').text()+'<br>Re_Note: '+$('#reference_note').val()+'</div><div style="font-size:11px !important;" class="col-md-6 pull-right"><p class="text-left">User: '+username+'<br>Date: '+date(site.dateFormats.php_ldate, time)+'</p></div></h6></span>';
        ...
        $.each(sortedItems, function () {
            ...
            if (pos_settings.remote_printing != 1) {
                ...
                var bprTr = '<tr  class="row_' + item_id + ' item_row" data-item-id="' + item_id + '"><td colspan="2" class="">#'+(an-1)+' ' + " - " + item_name + '';
                for (var i = 0, len = comments.length; i < len; i++) {
                    bprTr += (comments[i] ? '<br> <b>*</b> <small>'+comments[i]+'</small>' : '');
                }
                bprTr +='<td>(' + formatDecimal(item_qty) + ' x ' + (item_discount != 0 ? '<del>'+formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val) + item_discount)+'</del>' : '') + formatMoney(parseFloat(item_price) + parseFloat(pr_tax_val))+ ')</td><td style="text-align:right;">'+ formatMoney(((parseFloat(item_price) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) +'(R)</td>';
                bprTr += '</td></tr>';
        ...
        if (pos_settings.remote_printing != 1) {
            ...
            var bill_totals = '';
            bill_totals += '<tr class="bold fitem"><td>'+lang.total+'</td><td style="text-align:right;">'+formatMoney(total)+'(R)</td></tr>';
            
            if(order_discount > 0 || product_discount > 0) {
                bill_totals += '<tr class="bold fitem"><td>'+lang.discount+'</td><td style="text-align:right;">'+formatMoney(order_discount+product_discount)+'(R)</td></tr>';
            }
            if (site.settings.tax2 != 0 && invoice_tax != 0) {
                bill_totals += '<tr class="bold fitem"><td>'+lang.order_tax+'</td><td style="text-align:right;">'+formatMoney(invoice_tax)+'(R)</td></tr>';
            }
            bill_totals += '<tr class="bold fitem"><td>'+lang.grand_total+'</td><td style="text-align:right;">'+formatMoney(gtotal)+'(R)</td></tr>';
            if(pos_settings.rounding != 0) {
                round_total = roundNumber(gtotal, parseInt(pos_settings.rounding));
                var rounding = formatDecimal(round_total - gtotal);
                bill_totals += '<tr class="bold fitem"><td>'+lang.rounding+'</td><td style="text-align:right;border-top:2px solid #000 !important;">'+formatMoney(rounding)+'</td></tr>';
                bill_totals += '<tr class="bold fitem"><td>'+lang.total_payable+'</td><td style="text-align:right;border-top:2px solid #000 !important;">'+formatMoney(round_total)+'</td></tr>';
            }
            bill_totals += '<tr class="bold fitem"><td>'+lang.items+'</td><td style="text-align:right;border-top:2px solid #000 !important;">'+(an - 1) + ' (' + (parseFloat(count) - 1) + ')</td></tr>';
